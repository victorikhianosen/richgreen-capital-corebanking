...
class MonnifyController extends Controller
{
    ...
    public function getBanks(){
       ...
       if(env('APP_MODE') == "test"){
           $response =  Http::get($this->url."v1/sdk/transactions/banks")->json();
$banknames =[];
  //return $response;
        if($response['responseCode'] == '0'){
            foreach($response['responseBody'] as $banks){
                array_push($banknames,["bank_name" => $banks["name"],"bank_code" => $banks["code"], "bank_logo" => url('/')]);
            }    
            return response()->json(['status' => true,'message' => 'Banks Fetch Successfully','data' => $banknames]);
        }else{
            return response()->json(['status' => true,'message' => 'Failed to Fetch Banks','data' => []]);
         }
       }else{
             $banknames =[];
    ...
    public function VeriyBankAccount(Request $r)
    {
        ...
        $response =  Http::get($this->url."v1/disbursements/account/validate?accountNumber=".$r->account_number."&bankCode=".$r->bank_code);
  ...
  public function initiateTransaction(Request $request)
    {
        $lock = Cache::lock('initrn-'.mt_rand('1111','9999'),3);
if($lock->get()){
       $this->logInfo("initializing transaction",$request->all());
        ...
        $monify = $request->amount + $monnifycharge;
...
$trnxid =  $this->generatetrnxref('mfy');
    ...
    public function transferToBankAccount(Request $request)
    {
       ...
       $this->logInfo("bank transfer",$request->all());
        ...
        $totalAmount = $request->amount + $tcharge->amount + $monnifycharge + $ocharge->amount;
        $monify = $request->amount + $monnifycharge;
        $charge = $tcharge->amount + $monnifycharge + $ocharge->amount;
            ...
            if($transaction->status == "approved" || $transaction->status == "failed"){
              ...
              if ($monfybal["status"] == false) {
                 return response()->json($monfybal, 406);
                   ...
                   $debitCustomer = $this->DebitCustomerandcompanyGlAcct(Auth::user()->id,$totalAmount,$monify,$getsetvalue->getsettingskey('outwardoptiongl'),'m','Bank Transfer via monnify',$request->platform,$usern.'(c)');
                  ...
                  // $bank = banks::where('bank_code', $request->bank_code)->first();
                   ...
                   $newdescription = empty($request->description) ? "From " .$usern : $request->description;
                     ...
                     $this->logInfo("transfer Url",$this->url."v2/disbursements/single");
                     
                   $authbasic = base64_encode($this->apikey.":".$this->sercetkey);
                   $bankTransfer = Http::withHeaders([
                       "Authorization" => "Basic ".$authbasic
                   ])->post($this->url."v2/disbursements/single",[
                       "amount" => $request->amount,
                       "reference" => $request->transaction_reference,
                       "narration" => $newdescription."-".Auth::user()->last_name." ".Auth::user()->first_name,
                       "destinationBankCode" => $request->bank_code,
                       "destinationAccountNumber" => $request->destination_account,
                       "currency" => "NGN",
                       "sourceAccountNumber" => $this->acctno,
                       "destinationAccountName" => $request->receipient_name
                   ])->json();
               ...
               if ($bankTransfer["responseCode"] == "0") {
                 ...
                 return response()->json(['status' => true, 'message' => 'Bank Transfer Successful']);
                    ...
                    // $reverd = $this->ReverseDebitTrnxandcompanyGlAcct(Auth::user()->id,$totalAmount,$monify,$request->transaction_reference,$getsetvalue->getsettingskey('outwardoptiongl'),'m','Transaction reversed',$request->platform,'trnsfer',$usern.'(c)',$dacct);
