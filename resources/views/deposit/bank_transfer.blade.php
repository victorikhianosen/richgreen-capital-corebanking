              ...
              <div class="col-md-12">
                <div class="panel widget">
                  ...
                  @include('includes.success')
                    ...
                    <form class="form-horizontal"  action="{{route('bnkTransfer')}}" method="post" role="form" id="trxtransfer" onsubmit="thisForm()">
                      @csrf
                        <div class="row">
                      ...
                      <div class="form-group" style="padding: 0 6px;">
                        <label>Amount</label>
                          <input class="form-control width-70" step="any" autocomplete="off" required="required" name="amount" type="number" id="amount" value="{{old('amount')}}">
                      </div>
                      <div class="form-group" style="padding: 0 6px;">
                      ...
                      <input type="hidden" name="customerid"  class="custm" autocomplete="off" value="">
                            ...
                            <div class="col-md-6 col-lg-6 col-sm-12" style="border: 1px solid #f4f4f4;">
                                <h4>Destination Account</h4><hr>
                                <div class="form-group" style="padding: 0 6px;">
                                    <!--<label>Bank</label>-->
                                    <select name="bank_code" id="bnkcde" autocomplete="off" class="form-control width-70 bnksls">
                                        <option selected disabled>Select Bank</option>
                                        @foreach ($banks as $bank)
                                        <option value="{{$bank->bank_code}}">{{ucwords($bank->bank_name)}}</option>
                                        @endforeach
                                    </select>
                                </div>
                                <div class="form-group" style="padding: 0 6px;">
                                    <label>Account Number</label>
                                    <input class="form-control width-70" required="required" name="destination_account" type="text" id="acno2" autocomplete="off" value="{{old('acno_two')}}">
                                    <img src="{{asset('img/loading.gif')}}" id="sttext2" style="display: none" alt="loading">
                                </div>
                                  <div id="cbl2" style="display: none; margin:10px 2">
                                    <p>Reciptient Name: <span id="acnme2"  style="font-weight: 700"></span></p>
                                  </div>

                                  <input type="hidden" name="receipient_name" class="custm2" autocomplete="off" value="">
                            </div>
                        </div>
...
//credit account
    $("#acno2").keyup(function(){
      ...
      let bnkval = $("#bnkcde").val();
     if(acnoval.length == 10){
        if(bnkval != " "){
            $.ajax({
        url:"{{route('verifybnkacct')}}",
        method:"get",
        data:{'account_number':acnoval,'bank_code':bnkval},
        beforeSend:function(){
          $("#sttext2").show();
        },
        success:function(data){
        if(data.status === '0'){
          ...
          $("#sttext2").hide();
          $("#cbl2").show();
          $("#acnme2").text(data.data.last_name+" "+data.data.first_name).addClass('text-primary');
          $(".custm2").val(data.data.last_name+" "+data.data.first_name);
          toastr.success(data.msg);
          }
        },
        error:function(xhr,status,errorThrown){
