                        ...
                        <table class="table table-bordered table-hover table-sm">
                          ...
                          <tbody>
                            <tr>
                              ...
                              <!--  <input type="checkbox" id="checkboxxs" value="1" autocomplete="off" name="cuurencybox" >-->
                              <!--  <label for="checkboxxs"></label>-->
                              <!--</div>-->
                              </td>  
                              ...
                              <tbody>
                                ...
                                <tr>
                                       ...
                                       @if (!empty($_GET['filter']) && $_GET['filter'] == true)
                                          ...
                                          <tr>
                                            ...
                                            @foreach ($expaccate as $item)
                                                    ...
                                                    <?php 
                                                    if(request()->zbalance == '1'){
                                                        $expgls = DB::table('general_ledgers')->select('id','gl_name','gl_code','currency_id')->where('account_category_id',$item->id)
                                                                                                ->where('status','1')
                                                                                              ->get();
                                                    }else{
                                                       $expgls = DB::table('general_ledgers')->select('id','gl_name','gl_code','currency_id')->where('account_category_id',$item->id)
                                    ...
                                    @if (!empty($_GET['filter']) && $_GET['filter'] == true)
                                        ...
                                        <?php 
                                        if(request()->zbalance == '1'){
                                            $incmacco = DB::table('general_ledgers')->select('id','gl_name','gl_code','currency_id')->where('status','1')->where('account_category_id',$item->id)
                                                                                  ->get();
                                        }else{
                                           $incmacco = DB::table('general_ledgers')->select('id','gl_name','gl_code','currency_id')->where('account_category_id',$item->id)
                                                                                  ->where('status','1')
                                                                                ->where('account_balance','!=','0')->get();
                                        }
                                        ?>
                                ...
                                @if (!empty($_GET['filter']) && $_GET['filter'] == true)
                                         ...
                                         <tr>
                                            ...
                                            <td align="right">
                                               ...
                                               $currencyid = $glincmitem->currency_id ?: '';
                                                        //$currency = $getdata->GetcurrencyExchge($currencyid);
                                                        $rate = !empty($currencyid) ? $currencyRates[$currencyid] : 1;

                                                $incmtrnx = $incmcrtrnx - $incmdbtrnx;
                                                $convertedIncamt = $filtercuury == '1' ? $incmtrnx * $rate : $incmtrnx;
                                                 $incmbal += $convertedIncamt;
                                                ?>
                                         ...
                                         <tr>
                                            ...
                                            <td align="right">
                                                ...
                                                $incmetrnx = $incmcrtrnx - $incmdbtrnx;
                                                $convertedIncmeamt = $filtercuury == '1' ? $incmetrnx * $rate : $incmetrnx;
                                                 $incmbal += $convertedIncmeamt;
                                                ?>
                                               <span style="text-align:right"> <b>{{number_format($convertedIncmeamt,2)}}</b></span>
                                       ...
                                       {{-- <tr>
                                        ...
                                        <td></td>
                                        <td align="right">0.00</td>
                                        <td align="right"><b style="font-weight:bold;text-align:right">{{number_format($incmbal,2)}}</b></td>
                                      </tr> --}}
                                       <tr>
                                          
                                        <td><h5 style="font-weight:bold;text-align:left">Total</h5></td>
                                        <td></td>
                                        @php
                                           $totdebit = $asstbal + $expbal;
                                           $totcrdit = $incmbal + $capbal + $libal
                                        @endphp
                                        <td align="right"><b style="font-weight:bold;text-align:right">{{number_format($totdebit,2)}}</b></td>
                                        <td align="right"><b style="font-weight:bold;text-align:right">{{number_format($totcrdit,2)}}</b></td>
                                      </tr>
                              </tbody>
                          </table>
                      </div>
...
<script>
   ...
   a.document.write('<body>  @if (!empty($_GET["filter"]) && $_GET["filter"] == true)<h3>Trial Balance for the period: {{date("d M, Y",strtotime($datefrom))." To ".date("d M, Y",strtotime($_GET["dateto"]))}}</h3>@else <h3>Trial Balance</h3>@endif');
